          ...
          items: {
            ...
            properties: {
              target: {
                ...
                description:
                  "Path like 'parties/party_id/property' (no leading slash)",
              },
              operation: {
                type: "string",
                enum: ["set", "add"],
...
export function validateDecisionResponse(data: any): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (data.proposal) {
    ...
    if (
      !Array.isArray(proposal.participants) ||
    ...
    if (!Array.isArray(proposal.effects)) {
      ...
      // Validate effects
      proposal.effects.forEach((effect: any, index: number) => {
...
export function validateEffect(effect: any): string[] {
  const errors: string[] = [];
  ...
  if (!effect.target || typeof effect.target !== "string") {
    ...
    if (effect.target.startsWith("/")) {
      errors.push(
        'target path must not start with "/" (use "parties/..." not "/parties/...")'
      );
    }
    ...
    if (
      ...
      errors.push(
        `Invalid party ID format: ${pathParts[1]}. Must contain only alphanumeric characters and underscores`
      );
    }
    ...
    if (
      ...
      errors.push(
        `Invalid region ID format: ${pathParts[1]}. Must contain only alphanumeric characters and underscores`
      );
    }
  }
...
export function validateJsonSafety(jsonString: string): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
